...
def yolo_image_detector(path):
    ...
    labels_path = os.path.sep.join(["..\yolo-coco-data", "coco.names"])
    ...
    weights_path = os.path.sep.join(["..\yolo-coco-data", "yolov3.weights"])
    config_path = os.path.sep.join(["..\yolo-coco-data", "yolov3.cfg"])
    ...
    text = "YOLO Objects Detection took: {:.6f} seconds".format(end - start)
    ...
    if len(final_boxes) > 0:
        ...
        for i in final_boxes.flatten():
            ...
            text_box = "{}: {:.2f}%".format(labels[int(class_numbers[i])], confidences[i] * 100)
